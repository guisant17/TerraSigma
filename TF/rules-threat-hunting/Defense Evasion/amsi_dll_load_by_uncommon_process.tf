resource "azurerm_sentinel_alert_rule_scheduled" "amsi_dll_load_by_uncommon_process" {
  ...
  description                = "Detects loading of Amsi.dll by uncommon processes - Legitimate third party apps installed in \"ProgramData\" and \"AppData\" might generate some false positives. Apply additional filters accordingly"
  severity                   = "Low"
  query                      = <<QUERY
