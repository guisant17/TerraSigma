resource "azurerm_sentinel_alert_rule_scheduled" "raspberry_robin_subsequent_execution_of_commands" {
  name                       = "raspberry_robin_subsequent_execution_of_commands"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Raspberry Robin Subsequent Execution of Commands"
  description                = "Detects raspberry robin subsequent execution of commands. - Unlikely"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
