resource "azurerm_sentinel_alert_rule_scheduled" "fakeupdates_socgholish_activity" {
  name                       = "fakeupdates_socgholish_activity"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "FakeUpdates/SocGholish Activity"
  description                = "Detects initial execution of FakeUpdates/SocGholish malware via wscript that later executes commands via cmd or powershell. - Unlikely"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
