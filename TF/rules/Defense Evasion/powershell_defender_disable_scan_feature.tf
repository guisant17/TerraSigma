resource "azurerm_sentinel_alert_rule_scheduled" "powershell_defender_disable_scan_feature" {
  ...
  description                = "Detects requests to disable Microsoft Defender features using PowerShell commands - Possible administrative activity - Other Cmdlets that may use the same parameters"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
