resource "azurerm_sentinel_alert_rule_scheduled" "files_with_system_process_name_in_unsuspected_locations" {
  ...
  description                = "Detects the creation of an executable with a system process name in folders other than the system ones (System32, SysWOW64, etc.). It is highly recommended to perform an initial baseline before using this rule in production. - System processes copied outside their default folders for testing purposes - Third party software naming their software with the same names as the processes mentioned here"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
