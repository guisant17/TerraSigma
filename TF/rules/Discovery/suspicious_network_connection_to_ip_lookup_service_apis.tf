resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_network_connection_to_ip_lookup_service_apis" {
  ...
  description                = "Detects external IP address lookups by non-browser processes via services such as \"api.ipify.org\". This could be indicative of potential post compromise internet test activity. - Legitimate use of the external websites for troubleshooting or network monitoring"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceNetworkEvents
